...
/* TargetCard 함수 전체를 수정 기능 포함 버전으로 교체 */
s = s.replace(
/function TargetCard\(\{ target, onUpdate, onInvite, onDelete \}\) \{[\s\S]*?\n\}\n\nfunction PastorFeedbackList/,
`function TargetCard({ target, onUpdate, onInvite, onDelete }) {
  ...
  const [editEvangelistName, setEditEvangelistName] = useState(
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-sm text-slate-500 mt-1">
                  {target.relation || "관계 미입력"} · 들것지기 {target.stretcherGroup || "미지정"} · 전도인{" "}
                  {target.evangelistName || target.ownerName}
                </div>
                {target.phone && (
